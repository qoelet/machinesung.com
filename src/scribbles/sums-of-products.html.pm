...
◊m-article{
  ...
  ◊p{In Haskell, lists are ◊em{homogeneous}, that is, they can only be instantiated to some given type. ◊code{["foo", "bar"] :: [String]} is ok, but ◊code{["foo", 3]} is not. We can however represent the latter as a datatype,}
  ◊m-code-haskell{
...
import           Generics.SOP
  ...
  ◊p{Next, let's think about some trivial data to work with. If we want to encode some information about our meals today, we could do it as ◊code{[food, drink]}, so today I might feel like ◊code{["Rice", "Water"]}. Putting that into some trivial datatype definitions,}
  ◊m-code-haskell{
-- Sum
data Drink
  = Soda
...
instance Generic Meal
  ...
  ◊p{So we can think of the choice between constructors as sums, and they are represented via ◊em{Peano numbers}, and the arguments of each constructor via a list like structure. We can then write down the representation of this in the type signature: ◊code{Rep Meal ~ SOP I '[ '[ Chow, Drink ], '[Chow], '[] ]},}
  ◊m-code-haskell{
...
mealFromOpt s = to @Meal (chooseMeal s)
  ...
  ◊p{And we convert back to a list of strings. Here, ◊code{SOP I (Code Meal) ◊string->symbol{cong} datatype Meal} and we can switch this into a homogeneous structure (a list of strings) with the constant functor `K` (◊code{SOP (K String) (Code Meal) ◊string->symbol{cong} [String]})}
  ◊m-code-haskell{
hmeal :: Rep Meal
